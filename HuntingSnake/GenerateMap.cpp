...
// Generate level 3
void play_match3(unsigned int x_pos, unsigned int y_pos, unsigned int height, unsigned int width, Point obs[], int& obs_nums, int& SCORE, int& LEVEL) {
...
void play_match4(unsigned int x_pos, unsigned int y_pos, unsigned int height, unsigned int width, Point obs[], int& obs_nums, bool up, Point const_obs[], int& const_obs_nums, int& SCORE, int& LEVEL) {
	...
	create_obstacle_4(x_pos, y_pos, height, width, obs, obs_nums, const_obs, const_obs_nums);
	// draw match board
	draw_matchBoard(x_pos, y_pos, height, width, SCORE, LEVEL, 2, 0, "", 1);
	// Move obstacle
	while (true) {
		move_obs(x_pos, y_pos, height, width, obs, obs_nums, up, const_obs, const_obs_nums);
